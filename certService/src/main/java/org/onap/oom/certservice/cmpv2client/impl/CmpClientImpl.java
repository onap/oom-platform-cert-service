...
public class CmpClientImpl implements CmpClient {
    ...
    private boolean isPasswordBasedMacAlgorithm(AlgorithmIdentifier protectionAlgo) throws CmpClientException {
        if (Objects.isNull(protectionAlgo)) {
            LOG.error("CMP response not contains Protection Algorithm field");
            throw new CmpClientException("CMP response not contains Protection Algorithm field");
        }
        return protectionAlgo.getAlgorithm().equals(PASSWORD_BASED_MAC);
    }
