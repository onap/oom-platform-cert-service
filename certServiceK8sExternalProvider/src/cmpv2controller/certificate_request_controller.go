...
import (
	...
	updater "onap.org/oom-certservice/k8s-external-provider/src/cmpv2controller/updater"
...
func (controller *CertificateRequestController) Reconcile(k8sRequest ctrl.Request) (ctrl.Result, error) {
	...
	updater := updater.NewCertificateRequestUpdater(controller.Client, controller.Recorder, certificateRequest, ctx, log)
	...
	if err := controller.Client.Get(ctx, issuerNamespaceName, &issuer); err != nil {
		controller.handleErrorGettingCMPv2Issuer(updater, log, err, certificateRequest, issuerNamespaceName, k8sRequest)
		return ctrl.Result{}, err
	}

	// 5. Check if CMPv2Issuer is ready to sing certificates
	if !isCMPv2IssuerReady(issuer) {
		err := controller.handleErrorCMPv2IssuerIsNotReady(updater, log, issuerNamespaceName, certificateRequest, k8sRequest)
		return ctrl.Result{}, err
	}
	...
	if !ok {
		err := controller.handleErrorCouldNotLoadCMPv2Provisioner(updater, log, issuerNamespaceName)
		return ctrl.Result{}, err
	}
	...
	if err := controller.Client.Get(ctx, privateKeySecretNamespaceName, &privateKeySecret); err != nil {
		controller.handleErrorGettingPrivateKey(updater, log, err, privateKeySecretNamespaceName)
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		controller.handleErrorFailedToDecodeCSR(updater, log, err)
		return ctrl.Result{}, err
	}
	...
	if err != nil {
		controller.handleErrorFailedToSignCertificate(updater, log, err)
		return ctrl.Result{}, nil
	}
	...
	if err := updater.UpdateCertificateRequestWithSignedCertificates(); err != nil {
		return ctrl.Result{}, err
	}
