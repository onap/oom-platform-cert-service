...
func Test_shouldPrepareAndVerifyCMPv2Issuer_whenRequestReceived(t *testing.T) {
	scheme := initScheme()
	issuer, secret := testdata.GetValidIssuerAndSecret()
	mockClient := getFakeClient(scheme, issuer, secret)
	fakeRequest := getFakeRequest()
	fakeRecorder := record.NewFakeRecorder(3)
	controller := getController(fakeRecorder, mockClient)
	...
	expectedProvisioner, err := controller.ProvisionerFactory.CreateProvisioner(&issuer, secret)
...
func getFakeClient(scheme *runtime.Scheme, issuer cmpv2api.CMPv2Issuer, secret v1.Secret) client.Client {
